...
DECODED_BASE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX")
TXID=$(echo "$DECODED_BASE" | jq -r '.txid')
...
# WRITE YOUR SOLUTION BELOW:
NUM_INPUTS=$(echo "$DECODED_BASE" | jq -r '.vin | length')
check_cmd "Input counting" "NUM_INPUTS" "$NUM_INPUTS"

NUM_OUTPUTS=$(echo "$DECODED_BASE" | jq -r '.vout | length')
...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(echo "$DECODED_BASE" | jq -r '.vout[0].value' | awk '{printf "%.0f", $1*100000000}')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(echo "$DECODED_BASE" | jq -r --argjson min 15000000 '.vout | map(select((.value*100000000|round) >= $min)) | .[0].n')
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(echo "$DECODED_BASE" | jq -r --argjson n "$UTXO_VOUT_INDEX" '.vout[$n].value' | awk '{printf "%.0f", $1*100000000}')
...
PAYMENT_BTC=$(awk -v a="$PAYMENT_AMOUNT" 'BEGIN{printf "%.8f", a/100000000}')
CHANGE_BTC=$(awk -v a="$CHANGE_AMOUNT" 'BEGIN{printf "%.8f", a/100000000}')
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(echo "$DECODED_TX" | jq -r '[.vin[].sequence < 4294967294] | any')
check_cmd "RBF verification" "VERIFY_RBF" "$VERIFY_RBF"

VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" '.vout[] | select(.scriptPubKey.address==$addr) | .value')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address==$addr) | .value')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '.vout[] | select(.scriptPubKey.address==$addr) | .n')
...
# Convert to BTC
CHILD_SEND_BTC=$(awk -v a="$CHILD_SEND_AMOUNT" 'BEGIN{printf "%.8f", a/100000000}')
...
SECONDARY_OUTPUT_VALUE=$(echo "$DECODED_SECONDARY" | jq -r '.vout[0].value' | awk '{printf "%.0f", $1*100000000}')
...
# Convert to BTC
TIMELOCK_BTC=$(awk -v a="$TIMELOCK_AMOUNT" 'BEGIN{printf "%.8f", a/100000000}')
